...
def compute_batch_training_loss(src_batch, tgt_batch, model_params, config):
    ...
    # 1. Shift Targets Right 
    # (A, B, C, EOF) is the gold standard we compare to at the end
    # Shifted autoregressive decoder (start, A, B, C) --> this is the input to the decoder during training 
    gold_targets = tgt_batch.clone()
    start_id = config['start_id']
    start_col = tgt_batch.new_full((tgt_batch.size(0), 1), start_id)
    tgt_batch = torch.cat((start_col, tgt_batch(:, :-1)), dim=1)

    # 2. Run the Transformer Forward Pass 
    vocab_size = config['vocab_size']
    num_heads = config['num_heads']
    token_weights = model_params['token_embedding'] 
    _, d_model = token_weights.size()

    src_embeddings = token_weights[src_batch] * math.sqrt(d_model)
    tgt_embeddings = token_weights[tgt_batch] * math.sqrt(d_model)

    src_seq_len = src_embeddings.size(1)
    tgt_seq_len = tgt_embeddings.size(1)
    max_seq_len = max(src_seq_len, tgt_seq_len)
    ...
    src_mask = (pad_id != src_batch).unsqueeze(1).unsqueeze(2) #(B, 1, 1, S_s)
...
def compute_candidate_scores(beam_scores, next_token_log_probs):
    ...
    return next_token_log_probs + beam_scores.unsqueeze(1)
...
def select_top_k_candidates(candidate_scores, k):
    ...
    num_beams, vocab_size = candidate_scores.size()

    flat_scores = candidate_scores.reshape(-1) # (num_beams * vocab_size,)
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    ...
    new_column = token_ids.unsqueeze(1)
    return torch.cat([parent_sequences, new_column], dim=1 )
...
def select_best_finished_beam(finished_sequences, finished_scores, alpha):
    ...
    penalty_scores = torch.tensor([
        score / compute_length_penalty(len(seq), alpha)
        for seq, score in zip(finished_sequences, finished_scores)
    ])

    best_index = torch.argmax(penalty_scores)
    return {
        "sequence": finished_sequences[best_index],
        "score": penalty_scores[best_index],
    }
